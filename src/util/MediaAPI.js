...
const addTag = (file_id, tag) => {
  ...
  return fetch(apiUrl + 'tags', settings, console.log("Settings: ", settings)).then(response => {
    return response.json();
  });
};

const getFilesByUser = (id) => {
  ...
  return fetch(apiUrl + 'media/user/'+id,settings).then(response => {
    return response.json();
  });
};


export {getAllMedia, getSingleMedia, login, register, getUser, getFilesByTag, checkUser, getFilesByUser, addTag};